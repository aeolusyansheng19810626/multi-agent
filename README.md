...
- **Hybrid B**（辩论+嵌套混合）：支持方/反对方在辩论前先召唤专项子Agent（性能/成本/安全/可维护性）收集数据，再基于数据发言。UI实时显示子Agent召唤状态（⚡性能/💰成本/🔒安全/🔧可维护性），论点更有依据。第2轮自动获取对方所有历史论点进行针对性反驳
...
| Hybrid B | 辩论+嵌套混合，支持方/反对方召唤子Agent收集数据后辩论 | ✅ 已上线 |
